...
const readAndroidData = async (isYesterday = false) => {
	...
	if ('requestPermissionsAsync' in Pedometer) {
		...
		const { granted } = await (
			Pedometer as typeof Pedometer & {
				requestPermissionsAsync: () => Promise<PedometerPermissions>;
			}
...
export const useYesterdayHealthData = () => {
	...
	const fetchYesterdayData = useCallback(async () => {
		try {
			setState((s) => ({ ...s, loading: true, error: null }));
			const res =
				Platform.OS === 'android' ? await readAndroidData(true) : await readIOSData(true);
			setState({
				steps: res.steps,
				calories: res.calories,
				distance: res.distance,
